...
public class LongListSerializer implements TypeSerializer<List<Long>> {
    ...
    public boolean serialize(ConfigEntry entry, String value, @Nullable String holds) throws IllegalAccessException {
        entry.reference().set(null, new ArrayList<Long>(Arrays.stream(value.split(",")).map(String::trim).map(Long::decode).toList()));
        return true;
    }
}
